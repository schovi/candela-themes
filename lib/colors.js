...
// Nearest xterm-256 palette index for Vim's cterm fallbacks.
export function hexToXterm256Index(hex) {
  const { r, g, b } = hexToRgb(hex);
  let closestIndex = 0;
  let closestDistance = Infinity;
  for (const [index, [xtermR, xtermG, xtermB]] of XTERM_256_COLORS.entries()) {
    const distance = (r - xtermR) ** 2 + (g - xtermG) ** 2 + (b - xtermB) ** 2;
