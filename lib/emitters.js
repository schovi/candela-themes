...
const customizeHtml = `<p>${CUSTOMIZE} — <a href="${EDITOR_URL}">${EDITOR_LINK_TEXT}</a>.</p>`;
...
const heroMarkdown = () => `![Candela preview](${shotUrl('sepia-paper')})`;
...
function zedStyle(theme, ansiMapping) {
  ...
  const syntax = {};
  for (const [name, token, style] of ZED_SYNTAX) {
    syntax[name] = { color: n(c[token]), ...style };
  }
  style.syntax = syntax;
  return style;
}
...
export function emitFullFamily(themes, ansiMapping, licenseContent, iconContent, version = DEFAULT_VERSION) {
  ...
  for (const theme of themes) {
    ...
    // Same document builder as the single-theme export: two hand-built copies drifted
    // once already (the packaged family silently lost semanticHighlighting).
    const doc = vscodeThemeDocument(theme, ansiMapping);
