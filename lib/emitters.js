...
function emitIntellijPluginXml(themes) {
  const providers = themes
    ...
    .join('\n');
  const lightCount = themes.filter((t) => t.mode !== 'dark').length;
  ...
  return [
    ...
    `  <description><![CDATA[${lightCount} light and ${darkCount} dark color themes for tired eyes — low-glare, low-saturation, AAA body contrast.]]></description>`,
...
function terminalFiles(theme, ansiMapping, tool, ext, emit) {
  return { files: [file(`${theme.id}.${ext}`, emit(resolveTerminal(theme, ansiMapping), theme))] };
}
...
export function emitFullFamily(themes, ansiMapping, licenseContent, iconContent) {
  const files = [];
  for (const theme of themes) {
    const terminal = resolveTerminal(theme, ansiMapping);
    for (const { tool, ext, emit } of FORMATS) {
      files.push(file(`${tool}/${theme.id}.${ext}`, emit(terminal, theme)));
    }
  }
  ...
  const vscodeFamilyPackage = vscodePackage(themes);
  ...
  files.push(file('intellij/src/main/resources/META-INF/plugin.xml', emitIntellijPluginXml(themes)));
