...
// Shared listing copy — the single source for every marketplace README and the
// JetBrains plugin description. See docs/marketplace-listing.md. No theme counts
// anywhere: adding a theme must never mean editing marketing copy.
const DESCRIPTION =
...
const shotUrl = (id) => `${REPO_URL}/raw/main/docs/screenshots/candela-${id}.png`;
const heroMarkdown = `![Candela preview](${shotUrl('sepia-paper')})`;
const previewMarkdown = [
  '## Previews',
  '',
  ...[['slate-mist', 'Slate Mist'], ['meadow', 'Meadow'], ['nocturne', 'Nocturne']].map(
    ([id, name]) => `![${name}](${shotUrl(id)})`,
  ),
].join('\n');
