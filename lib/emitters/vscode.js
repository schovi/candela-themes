...
function resolveEditor(theme, ansiMapping) {
  ...
  const colors = {
    ...
    // VS Code's newer activity bar paints a rounded pill behind the active item and,
    // left unset, inherits list.inactiveSelectionBackground. Stated explicitly and
    // opaque: the pill is the same "you are here" state as a selected list row.
    'activityBar.activeBackground': n(c.selection),
    ...
    // Blends of `selection`, not `lineHighlight` — a line stripe is ~1% off `bg` by
    // design, which makes a list row indistinguishable from an unselected one.
    'list.inactiveSelectionBackground': alpha(c.selection, '99'),
    'list.hoverBackground': alpha(c.selection, '66'),
