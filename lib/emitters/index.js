...
import { slackThemeFile } from './slack.js';
...
export const FORMAT_EMITTERS = [
  ...
  { tool: 'slack', label: 'Slack', emit: (theme) => ({ files: [slackThemeFile(theme)] }) },
...
const INSTALL_STEPS = {
  ...
  warp: (theme) => `Save \`candela-${theme.id}.yaml\` in \`~/.warp/themes/\`, restart Warp if needed, then select Candela ${theme.name} in the theme picker.`,
  slack: 'Open Preferences → Appearance → Custom theme → Import, paste the four-color line, then set Darker sidebars and Window gradient as noted.',
...
export function emitFullFamily(themes, ansiMapping, licenseContent, iconContent, version = DEFAULT_VERSION) {
  ...
  const slackFiles = themes.map((theme) => slackThemeFile(theme, 'slack/'));
