// The seven flat-file terminal formats: one file per theme, built from the shared
...
// Warp names the ANSI colors alphabetically rather than by terminal slot order.
const WARP_ANSI_ORDER = ['black', 'blue', 'cyan', 'green', 'magenta', 'red', 'white', 'yellow'];

function emitWarp(t, theme) {
  const ansi = Object.fromEntries(ANSI_ORDER.map((name, index) => [name, [t.ansi[index], t.ansi[index + 8]]]));
  const colors = (brightness) => WARP_ANSI_ORDER.map((name) => `    ${name}: '${ansi[name][brightness]}'`);
  return [
    `# Candela ${theme.name} — generated by scripts/generate.js, do not edit.`,
    `name: Candela ${theme.name}`,
    `accent: '${theme.colors.builtin}'`,
    `background: '${t.background}'`,
    `foreground: '${t.foreground}'`,
    `cursor: '${t.cursor}'`,
    `details: ${theme.mode === 'dark' ? 'darker' : 'lighter'}`,
    'terminal_colors:',
    '  normal:',
    ...colors(0),
    '  bright:',
...
export const TERMINAL_FORMATS = [
  ...
  { tool: 'warp', label: 'Warp', ext: 'yaml', emit: emitWarp },
];
