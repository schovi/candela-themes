...
static assets — no backend. Publish it to a public URL (**candela.schovi.cz**) so the themes
...
- **Cloudflare Pages git integration stays the deployer.** Push to `main` → CF builds from
  the repo → deploys to candela.schovi.cz. Keep it — simplest publish path, and it gives PR
...
- **GitHub Actions CI for pre-merge checks.** Add `.github/workflows/ci.yml` on
  ...
  themes/candela-themes.json`, `node scripts/validate.js`, `cd app && npm ci && npm run
...
- **Provision/configure the Pages project over the Cloudflare MCP**: connect GitHub repo
  `schovi/candela-themes`, production branch `main`, **root directory** `app`, **build
  command** `node ../scripts/validate.js && npm run build`, **build output directory** `dist`
  (relative to root dir), add custom domain `candela.schovi.cz` (CF creates the CNAME in the
...
- Once live, add the URL to `README.md` (intro/top) and set the GitHub repo **About →
  Website** field to `https://candela.schovi.cz` (`gh repo edit schovi/candela-themes
  --homepage https://candela.schovi.cz`, or a documented manual step if `gh` isn't available).
- **Log the host decision** as `D1` in `docs/decisions.md`: Cloudflare Pages + git
  integration + validation-gated build + candela.schovi.cz, and why (existing CF zone, static
...
- Load-bearing: `app/src/themes.ts` build-time JSON import must survive the CF + CI build
  (root dir `app`, reads `../../themes/candela-themes.json`; Vite `fs.allow: ['..']` already
...
- `base`/asset paths verified correct for hosting at the candela.schovi.cz root (default `/` confirmed, or adjusted with a one-line reason).
- The Cloudflare Pages project is provisioned via the Cloudflare MCP (repo connected, production branch `main`, root dir `app`, the gated build command, output `dist`, custom domain candela.schovi.cz) — or the exact remaining manual step is documented where MCP can't cover it.
- The live URL is surfaced: `README.md` links it and the GitHub repo About/website field is set to `https://candela.schovi.cz`.
...
(Cloudflare Pages / git-integration gated on `validate.js` + app build / candela.schovi.cz).
