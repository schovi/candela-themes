...
but today Candela only emits a bare `build/zed/candela.json` theme-family file — not a
...
- Extend the Zed emitter in `scripts/generate.js` so `build/zed/` is a valid theme
  ...
  the theme family file at the path Zed expects (`themes/candela.json`). Verify the
...
- New `scripts/package-zed.js`: assembles the validated extension from `build/zed/`
  ...
  `dist/candela-themes-zed-<version>.tar.gz`; pick one and document it. Must exit
...
- **Load-bearing**: `generate.js` zero-runtime-dep + `build/` determinism;
  `themes/candela-themes.json` untouched source of truth; `ansiMapping` reuse for
...
- `build/zed/` loads via **Extensions → Install Dev Extension** and the Candela themes
...
- `npm run package:zed` produces the documented artifact in `dist/`
  (clean `dist/zed/` dir or `dist/candela-themes-zed-<version>.tar.gz`), exits non-zero
