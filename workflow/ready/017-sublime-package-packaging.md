...
a zip Sublime loads directly; Package Control is the registry). Today Aurora emits
loose `build/sublime/aurora-<id>.sublime-color-scheme` files with no package wrapper.
...
- New `scripts/package-sublime.js`: zips `build/sublime/` into
  `dist/aurora-themes.sublime-package` (a plain zip renamed to that extension).
...
- Root `README.md`: the "Other editors" line says Sublime is one file per theme, not
  ...
  `dist/aurora-themes.sublime-package` into the Sublime `Installed Packages/` folder
...
- **Load-bearing**: `generate.js` zero-runtime-dep + `build/` determinism;
  `themes/aurora-themes.json` untouched; the shared TextMate scope table stays the
  syntax mapping.
...
- `npm run package:sublime` produces `dist/aurora-themes.sublime-package`, exits
  non-zero on failure, no interactive prompts.
- Installing the package (`.sublime-package` in `Installed Packages/`, or the files
  in `Packages/User/`) makes the Aurora schemes selectable and they apply correctly
