...
Today Candela emits flat `build/nvim/candela-<id>.lua` files, which don't drop into a
...
- Change the Neovim emitter in `scripts/generate.js` to write the colorschemes under
  `build/nvim/colors/candela-<id>.lua` (so `:colorscheme candela-<id>` resolves once the
...
### Packaging script
- New `scripts/package-nvim.js`: produces `dist/candela-themes-nvim-<version>.tar.gz`
...
- **Load-bearing**: `generate.js` zero-runtime-dep + `build/` determinism;
  `themes/candela-themes.json` untouched; the chosen Lua-colorscheme approach from
...
- `npm run build` writes `build/nvim/colors/candela-<id>.lua` for all 14 themes plus a
...
- Putting `build/nvim/` on the runtimepath (or installing via a plugin manager) makes
  `:colorscheme candela-<id>` load and apply correctly (spot-check a warm, a cool, and
  one experiment theme).
- `npm run package:nvim` produces `dist/candela-themes-nvim-<version>.tar.gz`, exits
